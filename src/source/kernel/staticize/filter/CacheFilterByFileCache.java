...
import source.kernel.cache.CacheException;
import source.kernel.cache.FileCache;
 ...
 * @author Hai Thomson
 */
public class CacheFilterByFileCache implements Filter {
	// Header
	...
	private static final HashMap love = new HashMap<String, Long>();
	...
	private int fragment = CacheFilterByFileCache.FRAGMENT_AUTODETECT;
	...
	private int nocache = CacheFilterByFileCache.NOCACHE_OFF;

	// HTTP头信息控制,默认开启
	// 不是 LAST_MODIFIED_INITIAL 不设置头信息
	private long lastModified = CacheFilterByFileCache.LAST_MODIFIED_INITIAL;
	// HTTP EXPIRES 头信息控制,默认开启
	// 不是 EXPIRES_TIME 不设置头信息
	private long expires = CacheFilterByFileCache.EXPIRES_TIME;
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
		...
		try {
			responseContent = (ResponseContent) FileCache.load(key);
		} catch (CacheException e) {
			ExceptionHandler.handling(e, ExceptionHandler.EXCEPTION_SHOW_ON, ExceptionHandler.EXCEPTION_LOG_OFF, ExceptionHandler.EXCEPTION_HALT_ON);
		}
		// 如果从内存取得静态过的页面则输出页面
		...
		if (isCacheableInternal(cacheResponse)) {
			...
			// 缓存提前失效 1000 毫秒
			try {
				if (love.get(key) == null || System.currentTimeMillis() - (long)love.get(key) > 1L * 1000L) {
					love.put(key, System.currentTimeMillis());
					// System.out.println(new Date() + "\t刷新缓存");
					FileCache.put(key, cacheResponse.getContent(), this.time * 1000L - 1000L);
				}
			} catch (CacheException e) {
				ExceptionHandler.handling(e, ExceptionHandler.EXCEPTION_SHOW_ON, ExceptionHandler.EXCEPTION_LOG_OFF, ExceptionHandler.EXCEPTION_HALT_ON);
			}
	...
	public boolean isCacheable(ServletRequest request) {
		...
		if (cacheable) {
			...
			if (nocache == CacheFilterByFileCache.NOCACHE_SESSION_ID_IN_URL) {
				// 不支持,session in url
	...
	protected void makeRequestFiltered(FilterConfig filterConfig) {
		if (config != null && config.getFilterName() != null && !config.getFilterName().equals("")) {
			requestFiltered = CacheFilterByFileCache.REQUEST_FILTERED + config.getFilterName();
		} else {
			// 不被覆盖的情况下，this.getClass().getSimpleName() 值为 “子类名”
			requestFiltered = CacheFilterByFileCache.REQUEST_FILTERED + this.getClass().getSimpleName();
		}
	}
